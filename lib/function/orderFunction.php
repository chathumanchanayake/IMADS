...
//Getting the address during the order process
...
///////////////////////////////////this function set use to order details view //////////////////////////////
  ...
  //this function use to get order data to detail form
...
function productlist($iid){
 ...
 $sqlResult = $this->dbResult->query($sqlSelect);
  ...
  if($nor > 0){
    ...
    {
      $pid = $rec['product_Id'];
      $sqlSelect2 = "SELECT * FROM ebook_tbl WHERE id = '$pid';";
      //lets check the errors 
       if($this->dbResult->error){
       echo($this->dbResult->error);
       exit;
...
$pdf="";
    ...
    {
     {
      $pdf='<a id="pdfdwn" disabled href="../../lib/'.$rec2['doc'].'" download> Download</a>';}}
          ...
          $rec1 = $sqlResult1->fetch_assoc();
    {
      ...
      <div class="col-4">
          <p>'.$rec1['product_Category'].'</p>'.$pdf.'
          <button type="button" Onclick="rate(\''.$rec1['product_Id'].'\')" class="btn btn-warning">RateProduct</button>
      </div>
    </div>');
    }
  }
...
//   $sqlSelect = "SELECT * FROM repair_tbl WHERE user_id = '$sid' AND d_status = 0;";
//    //lets check the errors 
//     if($this->dbResult->error){
//     echo($this->dbResult->error);
//     exit;
...
//   {
//   if($rec['respons']=="")
//           { echo('<div class="card border-danger mb-3" id="sellItems" onclick="moredetailsservice(\''.$rec['repair_id'].'\')" style="max-width: 20rem;">
//             <div class="card-header">'.$rec['order_id'].'</div>
//             <div class="card-body">
...
//   echo('<div class="card border-danger mb-3" id="sellItems" onclick="homepage()" style="max-width: 20rem; max-height:20rem">
//   <img src="../upload/ui/04.png" width="100px" style="display: block; margin-left: auto; margin-right: auto; margin-top:40px; margin-bottom:5px;" >
//   <p style="text-align:center;margin-bottom:20px;">Add New Repair request<p>
// </div>');
// }
// }
...
//               $sqlSelect1 = "SELECT * FROM repair_tbl WHERE repair_id ='$iid' AND d_status = 0;";
//           //lets check the errors 
//             if($this->dbResult->error){
//             echo($this->dbResult->error);
//             exit;
...
//   }
  ...
  $sqlResult = $this->dbResult->query($sqlSelect);
   ...
   $sqlResult = $this->dbResult->query($update1);
    ...
    //this funtion to delete order details
    function rateorder($id, $rate, $user){

      $autonumber = new AutoNumber;
      $oiid = $autonumber -> NumberGeneration("feedback_id","feedback_tbl","FED");
        //insert order to databace
        $sqlInsert = "INSERT INTO feedback_tbl VALUES('$oiid','$user','$id','$rate','',0);";
    ...
    while($rec = $sqlResult->fetch_assoc()){
      $delid=$rec['dilevery_Id'];
        ...
        <tr>
          ...
          <td>'.$rec['order_date'].'</td>
          <td>'.$rec['order_price'].'</td>
          <td><button type="button" class="btn btn-success" onclick="offlinepay(\''.$rec['order_Id'].'\')">Pay Now</button></td>
    ...
    while($rec = $sqlResult->fetch_assoc()){
      $delid=$rec['dilevery_Id'];
        ...
        <tr>
          ...
          <td><button type="button" class="btn btn-warning" onclick="declare(\''.$rec['order_Id'].'\')">Decline</button></td>
       </tr>
              ');
...
public function tostore() {
    ...
    $sqlResult = $this->dbResult->query($sqlSelect);
    if (!$sqlResult) {
        echo "Error in query: " . $this->dbResult->error;
        exit;
    }
    ...
    if ($nor > 0) {
        while ($rec = $sqlResult->fetch_assoc()) {
            ...
            $sqlSelect1 = "SELECT * FROM delivery_tbl WHERE delivery_Id = ? AND d_status = 0 AND order_transport = 0 AND order_conform = 1 AND order_payment = 1 AND order_deliverd = 0;";
            $stmt1 = $this->dbResult->prepare($sqlSelect1);
            $stmt1->bind_param('s', $delid);
            $stmt1->execute();
            $sqlResult1 = $stmt1->get_result();
            ...
            if ($nor1 > 0) {
                while ($rec1 = $sqlResult1->fetch_assoc()) {
                    ...
                    $sqlSelect2 = "SELECT * FROM order_item_tbl WHERE d_status = 0 AND order_Id = ? ORDER BY id ASC;";
                    $stmt2 = $this->dbResult->prepare($sqlSelect2);
                    $stmt2->bind_param('s', $oid);
                    $stmt2->execute();
                    $sqlResult2 = $stmt2->get_result();
                    ...
                    if ($nor2 > 0) {
                        while ($rec2 = $sqlResult2->fetch_assoc()) {
                            ...
                            $sqlSelect3 = "SELECT * FROM store_tbl WHERE d_status = 0 AND product_Id = ?;";
                            $stmt3 = $this->dbResult->prepare($sqlSelect3);
                            $stmt3->bind_param('s', $proid);
                            $stmt3->execute();
                            $sqlResult3 = $stmt3->get_result();
                            ...
                            $item .= '<tr><th>' . htmlspecialchars($rec2['product_Id']) . '</th><th>' . htmlspecialchars($rec2['quantity']) . '</th><td>sec' . htmlspecialchars($rec3['product_Sec']) . ', Row' . htmlspecialchars($rec3['product_Rpw']) . ', Col' . htmlspecialchars($rec3['product_Col']) . ', Lev' . htmlspecialchars($rec3['product_Lev']) . '</td></tr>';
                        }

                        echo('
                        <tr>
                          <th>' . htmlspecialchars($rec['order_Id']) . '</th>
                          <th>
                          ...
                          <td><button type="button" class="btn btn-success" onclick="rady(\'' . htmlspecialchars($rec['order_Id']) . '\')">Order Ready</button></td>
                       </tr>
                        ');
                    }
                }
    ...
    while($rec = $sqlResult->fetch_assoc()){
      ...
      $userid=$rec['user_Id'];
        ...
        <tr>
          ...
          <td>'.$rec['address'].'</td>
          <td><button type="button" class="btn btn-warning" onclick="deliverd(\''.$rec['order_Id'].'\')">Delivered</button></td>
       </tr>
              ');
...
public function deliverlist2($searchData){
